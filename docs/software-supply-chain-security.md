...
| AWS archive infrastructure | Terraform 1.15.8 and AWS provider 6.58.0 declare Object Lock/KMS/IAM/CloudTrail controls; the signed provider lock covers Linux and Windows, and CI executes only backend-free validation plus mocked positive/negative plans | committed provider lock, static policy tests, `terraform validate`, and five non-applying `terraform test` plans |
...
| Updates | Dependabot proposes weekly grouped Python updates and weekly Actions, Docker, and Terraform provider updates | reviewed pull requests |
