...
| CORE-F-076 | The repository shall provide separately renderable Kubernetes foundation, admission, migration, and workload targets so cluster policy precedes schema migration and application rollout. | Kustomize render and policy tests |
...
| CORE-F-083 | Each promotion environment shall retain versioned evidence binding the environment, source SHA, image digest, rendered foundation/admission/migration/workload hashes, resource counts, and timestamps. | Evidence contract and manifest-promotion tests |
...
| CORE-NF-059 | Release least privilege | repository contents remain read-only; the approval job cannot publish, and only the dependent reusable-builder call receives package, OIDC, and attestation write permissions; registry authentication uses and then removes the short-lived job token |
...
| CORE-NF-061 | Provenance trust policy | promotion accepts only SLSA provenance signed for this repository by `.github/workflows/reusable-release-builder.yml`, with the requested source SHA, `refs/heads/main`, and no self-hosted signing runner |
