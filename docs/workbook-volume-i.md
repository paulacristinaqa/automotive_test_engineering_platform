...
| AWS S3 Object Lock adapter | Implemented provider adapter slice | Pre-upload restore, expected account, assumed role, conditional `COMPLIANCE` write, exact KMS key, version metadata, full read-back, and normalized evidence; live AWS foundation pending |
| Automated verification | Implemented | 169 fast tests plus expanded disposable black-box, alert-delivery, restore-drill, sealed release-archive/export-gate/AWS-adapter, and workflow-policy scenarios, 27 Android tests, Ruff, strict mypy, Android lint/build, Kustomize rendering, and integration/security/release/promotion CI workflows |
...
| R-019 | A deterministic seal, restore gate, normalized receipt, AWS S3 Object Lock adapter, and revocation procedure exist, but Actions retains evidence for only 90 days and no AWS archive foundation or live admission/revocation exercise is configured. | Long-lived automotive evidence may still expire or become undiscoverable, and fake-client evidence cannot prove real account, retention, IAM, KMS, CloudTrail, or recovery controls. | Provision and independently review the archive account/bucket/KMS/OIDC/CloudTrail boundary, run live upload/denial/restore acceptance, maintain an external revocation catalogue, install admission, and retain positive/negative/revocation evidence. | High |
