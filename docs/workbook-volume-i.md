...
| 0.43.0 | 12 August 2026 | Added a command-driven deterministic simulation clock and persisted `parked → ready → driving → parked` transition engine with vehicle-scoped idempotency, optimistic versioning, replay metadata, audit, and outbox evidence | 198 fast Python tests, Ruff, strict mypy, API/state-machine/retry/conflict coverage, migration `0014`, structural/a11y/visual workbook checks, and disposable integration delegated to hosted CI |
...
| Deterministic vehicle simulation | transition engine in `src/atep/vehicles/`, migration `0014_deterministic_vehicle_simulation.py`, `tests/test_digital_vehicle_state.py`, and Volume II requirements/roadmap |
