...
| Immutable provider export gate | Implemented provider-neutral validation slice | Exact bounded provider evidence, local full restore, key/version/checksum/size/retention/encryption/identity/audit/time validation, and normalized non-replacing receipt; live adapter pending |
| Automated verification | Implemented | 153 fast tests plus expanded disposable black-box, alert-delivery, restore-drill, sealed release-archive/export-gate, and workflow-policy scenarios, 27 Android tests, Ruff, strict mypy, Android lint/build, Kustomize rendering, and integration/security/release/promotion CI workflows |
...
| R-019 | A deterministic sealed object, restore gate, provider contract, normalized export receipt, and revocation procedure exist, but Actions retains evidence for only 90 days and no immutable provider/adapter or live admission/revocation exercise is configured. | Long-lived automotive evidence may still expire or become undiscoverable, and an unconfigured cluster may accept a digest without current trust or revocation enforcement. | Select independently governed locked-WORM storage, implement the short-lived provider adapter, retain live export/read-back receipts, maintain an external revocation catalogue, schedule clean-host restores, install the controller, and retain positive/negative/revocation evidence. | High |
