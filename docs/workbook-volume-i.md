...
| Automated verification | Implemented | 175 fast tests plus expanded disposable black-box, alert-delivery, restore-drill, sealed release-archive/export-gate/AWS-adapter/Terraform-policy, and workflow scenarios, five mocked Terraform plans, 27 Android tests, Ruff, strict mypy, Android lint/build, Kustomize rendering, and integration/security/release/promotion CI workflows |
...
| R-019 | A deterministic seal, restore gate, normalized receipt, AWS adapter, and Terraform archive foundation exist, but Actions retains evidence for only 90 days and no protected AWS apply, live archive acceptance, or admission/revocation exercise is configured. | Mock/static evidence cannot prove real account ownership, external audit policies, OIDC claims, effective IAM/KMS controls, retained object behavior, cost, or clean-host recovery. | Independently review the account and external controls, protect two-reviewer plan/apply, simulate IAM denials, then retain live upload/repeat-write/delete/shorten/bypass/CloudTrail/restore evidence before production use. | High |
